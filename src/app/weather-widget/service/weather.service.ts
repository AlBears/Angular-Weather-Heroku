...
export class WeatherService {
    ...
    getCurrentWeather(lat: number, long: number): Observable<any> {
        const url = `${process.env.FORECAST_ROOT}${process.env.FORECAST_KEY}/${lat},${long}`;
    ...
    getLocationName(lat: number, long: number): Observable<any> {
        const url = process.env.GOOGLE_ROOT;
        const queryParams = `?latlng=${lat},${long}&key=${process.env.GOOGLE_KEY}`;
